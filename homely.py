...
def generate_logic_html(config: Config) -> str:
    """Generate the expandable explanation appended to each HTML email."""

    slots = int(config.get("AGILE_DHW_SLOTS", "4"))
    ...
    dhw_slot_kwh = dhw_load_kw * 0.5

    return f"""
<details class="logic">
<summary>▶ How this scheduler works</summary>

<div class="logic-body">

<h3>1. What the program is trying to do</h3>
<p>
The scheduler chooses the <b>{slots} best 30-minute periods</b> in which
Homely should allow domestic-hot-water heating. It does not simply choose
the lowest Octopus Agile prices. It also takes forecast solar radiation into
account, so a period with a slightly higher grid price may still be better if
a large proportion of the heat pump's electricity is expected to come from PV.
</p>

<h3>2. Homely login and house discovery</h3>
<p>
The program logs into Homely using OAuth 2.0 Authorization Code with PKCE.
It then calls <code>userInfo</code> to discover the user ID,
<code>getUser</code> to discover the house ID,
<code>getSettings</code> to read DHW settings and timezone, and
<code>getAllSchedules</code> to read the live hot-water schedule.
No Homely access token, user ID, house ID, hot-water setpoint or schedule ID
is hard-coded.
</p>

<h3>3. Which day is changed</h3>
<p>
A normal scheduled run updates <b>tomorrow's</b> hot-water timetable.
The <code>--today</code> option changes today's timetable instead.
Only the target weekday is replaced; the other six days are preserved from
the live Homely schedule.
</p>

<h3>4. Agile electricity prices</h3>
<p>
The program obtains the published Octopus Agile half-hour unit rates for the
target date using the configured product and tariff for this profile.
</p>

<h3>5. Solar forecast</h3>
<p>
The forecast is obtained from Open-Meteo for
<b>{latitude:.5f}, {longitude:.5f}</b> using
<b>shortwave solar radiation</b> in watts per square metre (W/m²).
Hourly forecast values are interpolated to the 30-minute Agile periods.
</p>
<p>
The configured PV array is <b>{solar_array_kwp:.2f} kWp</b> with a
<b>{solar_performance:.0%}</b> performance factor.
</p>

<h3>6. Converting W/m² into predicted PV energy</h3>
<pre>
solar_factor = min(max(radiation_Wm2 / 1000, 0), 1)

predicted_solar_kW =
    solar_array_kWp × solar_factor × solar_performance

predicted_solar_kWh =
    predicted_solar_kW × 0.5 hours
</pre>

<h3>7. DHW electricity requirement</h3>
<p>
The heat pump's configured DHW electrical input is
<b>{dhw_load_kw:.2f} kW</b>. Over 30 minutes:
</p>
<pre>
DHW slot energy = {dhw_load_kw:.2f} kW × 0.5 h
                = {dhw_slot_kwh:.2f} kWh
</pre>

<h3>8. Predicted grid requirement</h3>
<pre>
grid_needed_kWh =
    max(DHW_slot_kWh - predicted_solar_kWh, 0)
</pre>
<p>
This is why the report can legitimately show <b>Grid 0.00 kWh</b>.
For this profile, if predicted PV during the half hour is at least
<b>{dhw_slot_kwh:.2f} kWh</b>, the model assumes the entire DHW electrical
requirement can be supplied by PV.
</p>

<h3>9. Solar-adjusted electricity price</h3>
<pre>
expected_cost_p =
    grid_needed_kWh × Agile_price_p_per_kWh

adjusted_price_p_per_kWh =
    expected_cost_p / DHW_slot_kWh
</pre>
<p>
A slot can therefore have a relatively high raw Agile price but an adjusted
price of zero when predicted PV covers the full DHW load.
</p>

<h3>10. How the winning slots are selected</h3>
<p>
All half-hour periods are sorted by <b>adjusted price</b>, lowest first.
The best <b>{slots}</b> periods are selected. Adjacent selected periods are
merged into a single Homely window.
</p>

<h3>11. What is sent to Homely</h3>
<p>
The program creates a complete hot-water schedule containing the unchanged
live entries for the other six weekdays and the newly calculated entries for
the target weekday, then sends it with <code>setHotWaterSchedule</code>.
With <code>--dry-run</code>, the proposed schedule is reported and emailed
but nothing is written to Homely.
</p>

<h3>12. Important current assumption</h3>
<p>
<b>All predicted PV generation is currently assumed to be available to the
DHW heat pump.</b> Normal household electricity consumption is not yet
deducted before allocating solar to DHW. Good-solar periods can therefore be
scored more favourably than they would be if the house were simultaneously
using significant electricity.
</p>
<p>
A future refinement would subtract an estimated or measured household base
load before calculating the PV available to DHW.
</p>

<h3>13. What the report figures mean</h3>
<p>
<b>Agile</b> — published Octopus price.<br>
<b>Radiation</b> — forecast shortwave radiation in W/m².<br>
<b>Solar</b> — predicted PV generation during the 30-minute period.<br>
<b>Grid</b> — predicted grid energy required by DHW after solar.<br>
<b>Adjusted</b> — effective grid price of the DHW workload after solar.
</p>
...
"""
    ...
    msg = EmailMessage()
    msg["Subject"] = subject
    msg["From"] = formataddr((f"{config.profile} {email_from_name}", email_from))
...
def main() -> None:
    ...
    parser.add_argument(
        "--env-file",
        default=str(Path(__file__).with_name("homely.env")),
        help="Config file path; default: homely.env beside this script",
    )
